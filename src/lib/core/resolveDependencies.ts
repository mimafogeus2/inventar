...
const createTuplesFromTransformers = (valueTuple: InventarEntryTuple, [currentTransformer, ...restOfTransformers]: InventarTransformersSequence = []) => {
  if (!currentTransformer) {
    return [valueTuple]
  }

  const transformerFunction = isTransformerObject(currentTransformer) ? currentTransformer.transformer : currentTransformer
  ...
  while (processQueue.length && cycleDetect <= processQueue.length) {
    ...
    try {
      if (doesGlobalTransformersExist || isValueObject(rawValue)) {
        ...
        flattenedTransformedValues.forEach(([newName, newValue]) => { resolvedConfig[newName] = newValue })
      } else {
